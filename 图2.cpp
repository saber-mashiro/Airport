...
for(int i=0;i<vernum;i++){
    ...
    for(int i=0;i<arcnum;i++){
      ...
      while(q->next!=NULL){
      	if(q->next->veradj>p->veradj)break;
      	q=q->next;}
      	p->next=q->next;
      	q->next=p;
    //arc[i].head=j;
...
int Graph::topoorder(){
    ...
    for(int i=0;i<n;i++)
      if(top==-1){cout<<"unworkable project"<<endl;delete[] count;return 0;}
      else{
          int j=top;
          top=count[top];
          //cout<<ver[j].name<<' ';
...
void Graph::shortestpath(int t){
  ...
  for(int i=0;i<n;i++){
   ...
   for(int j=0;j<n;j++){
       ...
       int ldis=10000;
       for(int i=0;i<n;i++)
        if(dis[i]>0&&dis[i]<ldis&&s[i]==0){ldis=dis[i];u=i;}
        if(ldis==10000)break;
        s[u]=1;
        p=ver[u].head1->next;
   }
